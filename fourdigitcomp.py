...
#exclude cue number and 5000s
exclude_set1 = range(5000,5999)
exp_numbers = list(num for num in numbers if num not in exclude_set1)
exp_numbers_fnl = random.sample(exp_numbers,10)

#generate random numbers for mil_cen_dec_uni block
#repeat above process
hundred_numbers = list(range(5000,5999))
exclude_set2 = {5555}
...
exp = design.Experiment(name="4 Digit Comparison task", text_size=40)
control.initialize(exp)

#create cue using red colour 
cue = stimuli.TextLine("5555", text_colour=[255,0,0])
blankscreen = stimuli.BlankScreen()
#create first block
block1 = design.Block("mil_cen_dec_uni")
for number in TARGETS1:
    t = design.Trial()
    t.set_factor('number', number)
#set factors for correct responses
    t.set_factor('is_larger', number > 5555)
    t.set_factor('is_smaller', number < 5555)
...
block2 =design.Block("cen_dec_uni")
